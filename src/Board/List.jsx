...
function List({ boardId, postCategory, boardCate, autoRefresh }) {
    ...
    return (
        <div>
                ...
                <ul>
                    {newPost ? (
                        <li className="board-list-newpost">
                            <button onClick={() => newPostReset()}>
                                새 글이 추가 되었습니다! <br /> <span>갱신하기</span>
                            </button>
                        </li>
                    ) : (
                        ""
                        ...
                        boardList.map((item) => (
                            <li key={item.id} className="board-admin-list">
                                <button
                                    name="delete"
                                    id={item.id}
                                    className={adminDelete.includes(item.id) ? "board-delete-chkbox checked" : "board-delete-chkbox"}
                                    onClick={() => handelDeleteCheck(item.id)}
                                >
                                    &nbsp;
                                </button>
                                <Link to={`/read/${item.id}`} className="board-list-item">
                                    ...
                                    <p className="board-badge-wrap">
                                        <span className="board-badge-like">
                                            <img src={`${process.env.PUBLIC_URL}/btn/like.svg`} alt="댓글수" className="board-item-icon" />
                                            {item.total_like}
                                        </span>
                                        <span className="board-badge-like">
                                            <img src={`${process.env.PUBLIC_URL}/btn/comment.svg`} alt="댓글수" className="board-item-icon" />
                                            {item.comment_count}
                                        </span>
                                    </p>
                                </Link>
            ...
            <Pagination total={totalPosts} limit={postsPerPage} page={currentPage} setPage={setCurrentPage} />
